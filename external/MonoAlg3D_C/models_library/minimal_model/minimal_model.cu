...
// m => Modificado
    ...
    real tau_o = ((1.0 - h_o) * tau_o1 + h_o * tau_o2); // ok
    real tau_so = (tau_so1 + (tau_so2 - tau_so1) * (1.0 + tanh(k_so * (u - u_so))) / 2.0);// ok
    real tau_vminus = ((1.0 - h_v_minus)) * tau_v1minus + h_v_minus * tau_v2minus; // ok

    //real J_fi = -v * H * (u - theta_v) * (u_u - u) / tau_fi;
    real J_fi = (-v * H * (u - theta_v) * (u_u - (u-u_o)) / tau_fi);//ok m
    real J_so = ((u - u_o) * (1.0 - h_w) / tau_o + h_w / tau_so); // ok
    real J_si = -h_w * w * s / tau_si; // ok

    //rDY_[0] = -(J_fi + J_so + J_si) + stim_current;
    rDY_[0] = scalee*(-(J_fi + J_so + J_si)) + stim_current * scalee; // ok m
    // no da mayra não tem esse stim_current aqui, o stym entra em current_state de vm
    // current_state = [offset, 1.0, 1.0, 0.0]  # vm, v, w, s
    // current_state[0] += parameters['pico'] * scale
    ...
    rDY_[1] = ((1.0 - H) * (v_inf - v) / tau_vminus - H * v / tau_vplus); // ok
    //if (tau_v_rl > 1e-10) {
    ...
    real w_inf = ((1.0 - h_o) * (1.0 - (u - u_o) / tau_winf) + h_o * w_infstar); // ok m
    real tau_wminus = (tau_w1minus + (tau_w2minus - tau_w1minus) * (1.0 + tanh(k_wminus * (u - u_wminus))) / 2.0); // ok
    ...
    rDY_[2] = ((1.0 - h_w) * (w_inf - w) / tau_wminus - h_w * w / tau_wplus); // ok
    //if (tau_w_rl > 1e-10) {
    ...
    real tau_s = ((1.0 - h_w) * tau_s1 + h_w * tau_s2); // ok
    real s_inf_rl = (1.0 + tanh(k_s * (u - u_s))) / 2;

    rDY_[3] = (((1.0 + tanh(k_s * ((u - u_o)-u_s))) /2.0 - s) / tau_s); // ok m
    //if (tau_s > 1e-10) {
